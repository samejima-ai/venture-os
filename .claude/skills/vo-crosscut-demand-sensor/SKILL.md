...
実市場データが無い段階では「**検証の設計**（反証可能命題＋安価な検証法）」を THESIS に含める。実検証（LP deploy・スモークテスト等）は gtm_execution 昇格後に段階解放（credit-runway と同型）。design_only の献上物には「需要前提表（前提／反証命題／検証法／確度）」を含め、**verified_ratio が低い＝仮説段階であることを明示**する。
