...
## 配置規約 ── 可視 / ドット / 別repo（事業モデル依存の判断）

> 決定: VO Council（business・経営者/開発者/哲学者・2026-07-06）→ **選択肢3（ハイブリッド）採用**。将来の事業ごとに再判断する配置の判断要素として正式記録。judgment_confidence 0.75 / final_decision は人間（本記録が方針化）。

**原則: 機構は隠す・統治は見せる。**

| 対象 | 配置 | 理由 |
|---|---|---|
| 実行機構（`vo-*` SKILL） | `.claude/skills/`（ドット下・隠す） | 機械が読む config 層。人はほぼ読まない |
| 人が読む統治（`philosophy` / `THESIS` / `VENTURE-REGIME` / `DONT-VENTURE` / `INDEX`） | **可視の名前空間**（例: `venture/`） | 批准・レビュー・認識合わせで人が**読み返す**のが目的。隠すと目的と逆行。DH が `SPEC`/`DONT`/`philosophy` を可視で置くのと対称 |

**やらないこと**:
- **全ドット化（`.vo/` に統治文書を隠す）は不採用**。ツール（ripgrep/バンドラ/静的サイト生成）が既定でドットディレクトリを無視する実害があり、かつ git 追跡・GitHub 表示で**秘匿にもならない**（偽りの安心）。「OS層＝ドット」は実行機構（正しくドット）と統治法（正しく可視）の2種のメタを混同する。

**機微情報の分離は事業モデル依存**（Council minority_opinion）:
- 競争戦略の秘匿を要する事業でのみ **別repo分離（製品repo / 事業設計repo）** を選ぶ。ドット化は「同 repo 内で目立たなくする」だけで混在を解かず代替にならない。
- **透明性自体が堀**の事業（例: ai-gata ＝ 誠実信用・本物の分布）では、むしろ可視が V1 と整合 → **機微分離不要・`venture/` 可視のまま**。
- 判断は事業ごとに Council 再諮問（配置規約の C2・トレードオフ）。動機が「②機微情報の分離」に確定した事業のみ 別repo(4) へ切り替える。
